...
def follow_suggestion(request):
    if request.user.is_authenticated:
        ...
        all_user_suggestion = suggestion.annotate(followers_count=Count('followers')).order_by('-followers_count')
        suggestion = suggestion.annotate(followers_count=Count('followers')).order_by('-followers_count')[:3]
        return {'suggestion': suggestion , 'all_user_suggestion':all_user_suggestion}
